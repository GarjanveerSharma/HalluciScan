...
function createPanel() {
  ...
  panel.innerHTML = `
    <div class="hs-header">
      <div>
        <span class="hs-logo">👻 Ghost</span>
        <div class="hs-status-chip">Live scan</div>
      </div>
...
function updatePanel(result) {
  ...
  content.innerHTML = `
    <div class="hs-score-section">
      <div style="font-size: 16px; font-weight: bold; color: ${riskColor}; margin-bottom: 8px;">
        Ghost Verification Status
      </div>
      <div>
        <div class="hs-risk" style="color: ${riskColor}">${result.claims && result.claims.some(c => c.status === 'hallucinated') ? 'False/Hallucinated claims detected' : 'Content Verified'}</div>
      </div>
    </div>
...
function startObserver() {
  setInterval(async () => {
    ...
    const content = document.getElementById('hs-content');
    if (content) {
      content.innerHTML = '<div class="hs-loading">Analyzing response...</div>';
    }
    ...
    if (result && !result.error) {
      ...
      content.innerHTML = '<div class="hs-error">Analysis unavailable. Is the server running?</div>';
    }
  }, SCAN_INTERVAL);
}
