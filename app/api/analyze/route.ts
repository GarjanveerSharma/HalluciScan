  ...
  try {
    ...
    try {
      ...
      try {
        ...
        // Build unique AI-style report using Wikipedia data directly
        const allEvidence = evidenceResults.flat();
        const topSnippet = allEvidence.length > 0 ? allEvidence[0].snippet : '';
        ...
        if (topSnippet) {
           contextualAnswer = `Based on authoritative sources regarding ${topicsStr}, we found relevant information: "${topSnippet.substring(0, 200)}..."\n\n`;
        }

        geminiAnswer = `✨ Ghost AI Analysis\n\n${contextualAnswer}The submitted text contains ${claims.length} verifiable claim${claims.length !== 1 ? 's' : ''} cross-referenced against ${allEvidence.length} evidence source${allEvidence.length !== 1 ? 's' : ''}. \n\nReview the claim-by-claim breakdown below for detailed verification statuses.`;
      }
    }
